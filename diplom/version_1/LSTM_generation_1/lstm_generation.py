...
class LSTMRhymingPoetryGenerator:
    ...
    def _generate_free_line(
        ...
        for _ in range(max_attempts):
            # Выбираем случайное слово как последнее
            valid_words = [
            ...
            # Начинаем генерацию с этого слова (как последнего)
            tokens, words = [self.vocab["<bos>"], self.vocab[start_word]], [start_word]

            # Генерация в обратном порядке (справа налево)
            while len(words) < max_words:
                logits = self.model.predict(
                    np.array([tokens], dtype=np.int32), verbose=0
                )[0, -1, :]
            ...
            # Восстанавливаем нормальный порядок через _postprocess_line
            line = self._postprocess_line(words, min_words)
            if line:
                return line
