...
class BahdanauAttention(Layer):
    """Additive attention (Bahdanau) для языковой модели"""
    ...
    def call(self, inputs, **kwargs):
        ...
        query_with_time_axis = ops.expand_dims(query, axis=1)

        score = self.V(ops.tanh(
...
class LSTMRhymingPoetryGenerator:
    ...
    def load_and_train(self, filepath="poems_clean.txt", epochs=80):
        """обучение моделей"""
    ...
    def _build_model(
        ...
        def apply_attention_per_step(args):
            lstm_output = args

            batch_size = ops.shape(lstm_output)[0]
            seq_len = ops.shape(lstm_output)[1]

            return lstm_output

        attention_output, _ = attention([prev_output[:, -1, :], prev_output])

        combined = Dense(hidden_size, activation='tanh', name='attention_combine')(prev_output)
    ...
    def _generate_free_line(
        ...
        for _ in range(max_attempts):
            ...
            tokens, words = [self.vocab["<bos>"], self.vocab[start_word]], [start_word]
    ...
    def generate_poem(
        ...
        if start_line and len(start_line.strip()) >= 10:
            ...
            if clean_start and (last_word := self._extract_last_word(clean_start)):
                ...
                start_idx = 1
                self.metrics.add_line_metrics(clean_start)
        ...
        for i in range(start_idx, lines):
            ...
            if rhyme_letter not in rhyme_map:
                ...
                if rhyme_word:
                    ...
                    for j in range(len(poem) - 2, -1, -1):
                        if scheme[j] == rhyme_letter:
                            quality = self.metrics.calculate_rhyme_quality(poem[j], line)
        ...
        if poem:
            unique_lines = sum(1 for line in poem if self._is_unique_line(line))
            unique_rate = unique_lines / len(poem)
            ...
            self.metrics.add_poem_metrics(poem)
            return poem[:lines]
        return None
...
def main():
    ...
    if not generator.load_and_train("poems_clean.txt", epochs=80):
